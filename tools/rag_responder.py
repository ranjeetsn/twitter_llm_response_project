...
class HelpCenterAgent:
    ...
    @classmethod
    def load_pdf_py_pdf(cls, pdf_file: str):
        """
        Load documents from the given directory.
        """
    ...
    @classmethod
    def load_pdf_no_tables(cls, filename: str):
        """
        Load pdf files """
        doc_file = cls.load_pdf_py_pdf(filename)
        pdf_output = []
        for page_doc in doc_file:
            meta_data = page_doc.metadata
            pdf_docs = page_doc.page_content.split("\n")
            pdf_output += [Document(page_content = x, metadata = meta_data) for x in pdf_docs]
        return pdf_output
